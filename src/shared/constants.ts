...
export const appDescription = `
  This is a proof of concept app if for searching inventory data created by an external system.
`
...
export const userPrompt =
  'Analyze this image and return a stringified JSON object with every individual item in a root property called `items` with the following sub properties: `type` (what type of item is it), `brand` (item brand if known, empty string if not), `label` (descriptive label for the item if it was on a store shelf), `description` (description of the item and its appearance), `material` (material the item is made of), and `categories` (organizational categories and tags that would apply to the item). Also include a `visible_text` root property on the JSON object that contains an array of strings of written text found in the image.'
